...
const Modal = (props: modalOpen) => {
  ...
  const backgroundURLRef = useRef<HTMLInputElement | null>(null);
  const board = useSelector((state: RootState) => state.board);

  const createHandler = () => {
    if (titleRef.current && backgroundURLRef.current) {
      props.create(
        titleRef.current.value || board.title,
        backgroundURLRef.current.value || board.backgroundURL
      );
